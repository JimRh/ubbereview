...
from api.models import Shipment, Leg, SubAccount, MetricAccount, Package, RateLog
...
class Command(BaseCommand):
    ...
    def _create_daily(self, account: SubAccount, metric_date: datetime):
        ...
        data = {
            "creation_date": metric_date.replace(tzinfo=utc),
            "sub_account": account,
    ...
    def add_arguments(self, parser):
        parser.add_argument('start_date', type=str, help='2020-01-01')
        parser.add_argument('end_date', type=str, help='2020-12-31')
    ...
    def handle(self, *args, **options) -> None:
        ...
        test_start_date = copy.deepcopy(start_date)

        for account in sub_accounts:

            # test_start_date = datetime.datetime.strptime(account.creation_date.strftime("%Y-%m-%d"), "%Y-%m-%d")

            while test_start_date != end_date:

                self._create_daily(account=account, metric_date=test_start_date)

                self.stderr.write(
                    self.style.SUCCESS(f'Metric Account populated for: {account} - {test_start_date}')
                )
