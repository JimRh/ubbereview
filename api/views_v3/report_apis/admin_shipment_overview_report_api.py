"""
    Title: Admin Shipment Overview Report apis
...
from api.apis.reports.admin_shipment_overview_report import AdminShipmentOverviewReport
...
class AdminShipmentOverviewReportApi(UbbeMixin, APIView):
    ...
    @swagger_auto_schema(
        ...
        operation_id='Get Admin Shipment Overview Report',
        operation_description='Get Admin Shipment Overview report.',
        responses={
    ...
    def get(self, request, *args, **kwargs):
        """
            Get Admin Shipment Overview Report.
            :param request: request object
            :return: Metric Data
        """
        errors = []

        if not API.objects.get(name="AdminShipmentOverviewReportApi").active:
            connection.close()
            return Utility.json_error_response(
                code="3200", message="Admin Shipment Overview Report Api is not active.", errors=errors
            )
        ...
        if start_date is None or end_date is None:
            ...
            return Utility.json_error_response(
                code="3201", message="AdminShipmentOverviewReport: Missing 'start_date' & 'end_date' parameter.", errors=errors
            )

        shipments = self.get_queryset()

        try:
            shipment_overview_report = AdminShipmentOverviewReport().get_shipment_overview(
                shipments=shipments,
                file_name=f'admin_shipment_overview_{start_date}-{end_date}.xlsx'
            )
        except ViewException as e:
            connection.close()
            errors.append({"account": e.message})
            return Utility.json_error_response(
                code="3202", message="AdminShipmentOverviewReport: failed to create.", errors=errors
            )
