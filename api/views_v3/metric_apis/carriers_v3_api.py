"""
    Title: Metric Carrier Api views
...
from api.apis.metric_v3.metric_carriers_v3 import GetMetricCarriers
...
class MetricCarriersApi(UbbeMixin, APIView):
    """
        Get a Metric Carrier overview.
    """

    # todo - Do properly serializer for response.

    @staticmethod
    def _get_cached_data(metrics: GetMetricCarriers, sub_account: SubAccount, params: dict, metric_type: str) -> dict:
        """
    ...
    @swagger_auto_schema(
        ...
        operation_id='Get Metric Carrier Overview',
        operation_description='Get metric carrier overview which includes top carriers and top services.',
        responses={
            '200': "Metric data",
            '400': "Bad Request",
    ...
    def get(self, request, *args, **kwargs):
        ...
        if not account:
            ...
            return Utility.json_error_response(code="6800", message="Metric Carriers: Invalid params.", errors=errors)
        ...
        try:
            ...
            return Utility.json_error_response(code="6801", message="Metric Carriers: Account Not found.", errors=errors)

        metric_carriers = GetMetricCarriers()
        ...
        if not sub_account.is_bbe:
            ...
            try:
                metric_data = self._get_cached_data(
                    metrics=metric_carriers, sub_account=sub_account, params=params, metric_type="carriers"
                )
        ...
        try:
            ...
            return Utility.json_error_response(code="6802", message="Metric Carriers: Invalid param.", errors=errors)
        ...
        try:
            metric_data = metric_carriers.get_metrics(sub_account=sub_account, params=params)
        except ViewException as e:
            connection.close()
            return Utility.json_error_response(code=e.code, message=e.message, errors=e.errors)
